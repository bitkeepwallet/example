...
const initialize = async () => {
  try {
    ...
    ethersProvider = new ethers.providers.Web3Provider(window.bitkeep.ethereum, 'any');
  ...
  const onClickConnect = async () => {
    try {
      const newAccounts = await bitkeep.ethereum.request({
  ...
  const initializeAccountButtons = () => {
    ...
    sendFailingButton.onclick = async () => {
      try {
        const result = await bitkeep.ethereum.request({
    ...
    requestPermissionsButton.onclick = async () => {
      try {
        const permissionsArray = await bitkeep.ethereum.request({
    ...
    getAccountsButton.onclick = async () => {
      try {
        const _accounts = await bitkeep.ethereum.request({
          method: 'eth_accounts',
        });
    ...
    getEncryptionKeyButton.onclick = async () => {
      try {
        encryptionKeyDisplay.innerText = await bitkeep.ethereum.request({
    ...
    decryptButton.onclick = async () => {
      try {
        cleartextDisplay.innerText = await bitkeep.ethereum.request({
          method: 'eth_decrypt',
          params: [ciphertextDisplay.innerText, bitkeep.ethereum.selectedAddress],
        });
      } catch (error) {
  ...
  ethSign.onclick = async () => {
    try {
      ...
      const ethResult = await bitkeep.ethereum.request({
        method: 'eth_sign',
  ...
  personalSign.onclick = async () => {
    ...
    try {
      ...
      const sign = await bitkeep.ethereum.request({
  ...
  personalSignVerify.onclick = async () => {
    ...
    try {
      ...
      const ecRecoverAddr = await bitkeep.ethereum.request({
  ...
  async function getNetworkAndChainId() {
    try {
      const chainId = await bitkeep.ethereum.request({
        method: 'eth_chainId',
      });
      handleNewChain(chainId);

      const networkId = await bitkeep.ethereum.request({
        method: 'net_version',
      });
      handleNewNetwork(networkId);

      const block = await bitkeep.ethereum.request({
  ...
  if (isMetaMaskInstalled()) {
    bitkeep.ethereum.autoRefreshOnNetworkChange = false;
    getNetworkAndChainId();

    bitkeep.ethereum.autoRefreshOnNetworkChange = false;
    getNetworkAndChainId();

    bitkeep.ethereum.on('chainChanged', (chain) => {
    ...
    bitkeep.ethereum.on('chainChanged', handleNewNetwork);
    bitkeep.ethereum.on('accountsChanged', (newAccounts) => {
      bitkeep.ethereum
        .request({
          method: 'eth_getBlockByNumber',
          params: ['latest', false],
